...
def ctc_input_length(model, input_lengths):
    import keras.layers
...
def spectrogram_from_file(filename, step=10, window=25, max_freq=None,
                          eps=1e-14):
...
def save_model(save_dir, model, train_costs, validation_costs, validation_acc, index=None):
    ...
    np.savez(os.path.join(save_dir, 'costs.npz'), train=train_costs,
             validation=validation_costs, validation_acc=validation_acc)
...
def argmax_decode_1(prediction):
    ...
    for c in int_sequence:
        ...
        if c != 0:  # Blank
            tokens.append(c)
        c_prev = c
    print('prediction_int_sequence:'+str(int_sequence))
...
def argmax_decode(prediction, decode_fn, ctc_input_lens):
    ...
    input_length = input_length.flatten()
    print('y_pred_shape:'+str(y_pred.shape))
    out = decode_fn([y_pred, input_length])[0]
    # out = K.get_value(K.ctc_decode(y_pred, input_length, )[0][0])[:,:]
    print('ctc_decode_shape:'+str(out.shape))
    print('ctc_decode:'+str(out))
    text_list=[]
    for batch in out:
        tokens = []    
        for x in batch:
            ...
            if x != 0 :  # blank
                tokens.append(x)
        text = ''.join([index_map[x] for x in tokens])
...
def text_to_int_sequence(text):
    ...
    for c in text:
        ...
        # int_sequence.append(4562)
    # print("label_int_squence:"+str(int_sequence))
    # print('int_sequence:'+str(int_sequence))
    return int_sequence
